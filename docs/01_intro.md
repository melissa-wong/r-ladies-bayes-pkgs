...
I use the `bayesplot` package to demonstrate the steps for creating some of the most common diagnostic plots. I found this helpful as I was learning how to use each package. The `tidybayes` package is another good option for visualization of Bayesian models. However, I highly recommend the `shinystan` package; it will automatically create all of these diagnostic plots (and more) with an nice interactive web interface.
